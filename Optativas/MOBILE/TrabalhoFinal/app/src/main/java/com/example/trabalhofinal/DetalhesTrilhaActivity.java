...
public class DetalhesTrilhaActivity extends AppCompatActivity implements OnMapReadyCallback {
     ...
     *
     * <p>Este método executa a configuração inicial do mapa assim que ele se torna disponível.
     * As seguintes ações são realizadas em sequência:</p>
     * <ul>
     *   <li>Executa uma verificação de segurança para garantir que a trilha já foi carregada
     *       e que a atividade não está sendo finalizada, prevenindo erros.</li>
     *   <li>Atribui a instância do {@link GoogleMap} à variável de membro da classe.</li>
     *   <li>Define o tipo do mapa (ex: normal, satélite) com base no valor que foi salvo
     *       junto com a trilha.</li>
     *   <li>Remove qualquer padding interno do mapa para que ele ocupe todo o espaço
     *       disponível em seu contêiner (ex: um CardView).</li>
     *   <li>Chama o método {@link #desenharPercurso()} para renderizar a rota da trilha na tela.</li>
     * </ul>
     *
